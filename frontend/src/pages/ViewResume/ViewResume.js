...
const ViewResume = () => {
  ...
  const handleDownload = async (format) => {
    try {
      ...
      // Create standardized filename: firstname_lastname-tailored.ext
      const downloadFileName = lastName
        ? `${firstName}_${lastName}-tailored.${format}`.toLowerCase().replace(/ /g, '_')
        : `${firstName}-tailored.${format}`.toLowerCase().replace(/ /g, '_');
